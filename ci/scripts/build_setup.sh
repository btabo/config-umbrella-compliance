...
if [[ "$BREAK_GLASS" == "true" ]]; then
  ...
  docker login -u iamapikey --password-stdin "$ICR_REGISTRY_REGION.icr.io" < /config/IC_1416501_API_KEY
  ...
  ibmcloud login --apikey @/config/IC_1416501_API_KEY -r "$IBM_LOGIN_REGISTRY_REGION"
